...
function  stratapp:start( $doc as xs:string, $repository as xs:boolean) {
			let $content :=   
			if( fn:doc($doc)  ) 
								then 
									xslt:transform(
...
declare
  ...
  %output:method('xml')
  %output:omit-xml-declaration('no')
function  stratapp:planpresentation($doc as xs:string) {
			let $content :=   xslt:transform(
						db:open("data", $doc),
