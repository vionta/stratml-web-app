  ...
  <xsl:template match="nested[@type='dependency' and datereal/text()]"></xsl:template>
	...
	<xsl:template match="xf:instance[@id='default']">
		 <xf:instance xmlns="" id="default"  >
		 	<xsl:copy-of select="parse-xml($doc)" />
		...
		<xsl:template match="xf:instance[@id='config']">
		 <xf:instance xmlns="" id="config"  >
			<data>
			  ...
			  <source></source>
			  <path>http://localhost:8984</path>
			  <sourcedocument>http://localhost:8984/rest/data/template.xml</sourcedocument>
			  <filename><xsl:value-of select="$doc" /></filename>
			  <page>0</page>
			  <plan-load-path>http://localhost:8984/static/loadplan.htm</plan-load-path>
			  <local-plan-path>http://localhost:8984/static/localplan.html</local-plan-path>
			  <browse-path>http://localhost:8984/static/browse.htm</browse-path>
			</data>
	</xf:instance>
	</xsl:template>
